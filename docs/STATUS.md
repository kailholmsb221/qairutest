...
| Фокус-вид: увеличение ×1.12 через `viewBox`, слой без CSS-scale и без `will-change` | Вместо `scale(1.12)` на композитном слое | Слой с `will-change`/scale растрируется один раз и растягивается — план «плыл» при зуме; векторный zoom всегда чёткий |
